...
int main(int argc, char* argv[]) {
    ...
    // Testing Mersenne
    // HOW THE FUCKING FUCK IN HELL DOES IT WORK WITHOUT AN INCLUDE
    /*for(int i = 0; i<10;i++) {
        printf("MERS: %f, RAND: %f\n", mt_drand(), (double)rand()/RAND_MAX);
    }*/
    ...
    for(int step = 0; step<timesteps; step++) {
        ...
        for(int i = 0; i<(Ne+Ni); i++) {
            iVec[i] = iVec[i] + sumVec[i]; // This is easily modified to implement Ambroise-Levi. Values for biasing current are to be found somehow.
            
            // Izhikevic
            vVec[i] = vVec[i]+0.5*0.78*(0.04*pow(vVec[i], 2)+5*vVec[i]+140-uVec[i]+iVec[i]);
            vVec[i] = vVec[i]+0.5*0.78*(0.04*pow(vVec[i], 2)+5*vVec[i]+140-uVec[i]+iVec[i]);
